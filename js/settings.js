...
(function () {
	...
	var ElementView = Backbone.View.extend( {
		className: 'ultimate-fonts-element',
		template: wp.template( 'ultimate-fonts-element' ),
		events: {
			'click .ultimate-fonts-element__delete': 'delete'
		},
	...
	var AppView = Backbone.View.extend( {
		...
		el: '#ultimate-fonts-elements',

		// Delegated events for creating new elements.
		events: {
			'click #ultimate-fonts-add': 'create'
		},
		...
		initialize: function () {
			this.$button = this.$( '#ultimate-fonts-add' );

			this.listenTo( elements, 'add', this.add );

			// Get elements from WordPress settings.
			elements.add( Ultimate_Fonts.elements );
		},
